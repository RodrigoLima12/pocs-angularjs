...
// }
//
export const getUrlLogout = () => {
    return (
        NsjAuthInstance.nsjAuth
            ? NsjAuthInstance.nsjAuth.createLogoutUrl({redirectUri: window.location.href})
            : ''
    )
}
...
function _handleError(ctx: any, errorType: any, handler: HandlerFunction, error: Error) {
    ...
    if (typeof handler === 'function'&& typeof error == errorType) {
